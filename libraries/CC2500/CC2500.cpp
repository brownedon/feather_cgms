 ...
 *  MERCHANTABILITY or FITNEA3 FOR A PARTICULAR PURPOSE.  See the
...
#include <SPI.h>
int SPI_DELAY=1;  //0 works for rfduino
CC2500::CC2500()
{
}
...
void CC2500::init()
{
 Serial.println("CC2500 Init");
    ...
    reset();
	 Serial.println("CC2500 Reset Done");
...
//FSCTRL1 and MDMCFG4 have the biggest impact on sensitivity...
   ...
   WriteReg(REG_BSCFG, 0x6C);
	 ...
	 Serial.println("CC2500 Registers written");
}
...
// For status/strobe addreA3es, the BURST bit selects between status registers
// and command strobes.
